...
#         f"&fields=%7B%22incidents%22%3A%5B%22type%22%2C%22geometry%22%2C%22properties%22%5D%7D"
#     )
...
#         "HAZARD": "blue"
#     }
#     return colors.get(incident_type, "gray")


# # ------------------ UI ------------------
# st.set_page_config(page_title="Live Traffic Monitoring", layout="wide")
# st.title("🚦 Real-Time Traffic Monitoring App (Using TomTom API)")
# st.write("Enter any city/place name to view **live traffic incidents**.")

# place = st.text_input("Enter Place Name (Example: Hyderabad, Mumbai, Bengaluru):", "Hyderabad")

# if st.button("Search"):
#     lat, lon = get_coordinates(place)

#     if lat is None:
#         st.error("❌ Place not found. Try again.")
#     else:
#         st.success(f"✔ Found location: {place} (Lat: {lat}, Lon: {lon})")
...
#             try:
#                 inc_type = incident["properties"]["type"]
#                 coordinates = incident["geometry"]["coordinates"][0]
#                 description = incident["properties"].get("description", "No details provided")
#                 color = get_incident_color(inc_type)

#                 folium.Marker(
#                     location=[coordinates[1], coordinates[0]],
#                     popup=f"{inc_type}: {description}",
#                     icon=folium.Icon(color=color)
#                 ).add_to(traffic_map)
#             except:
#                 continue

#         st_folium(traffic_map, width=900, height=550)

#         st.subheader("📝 Traffic Incident Details")
#         if len(incidents) == 0:
#             st.info("✅ No traffic issues detected nearby!")
#         else:
#             for inc in incidents:
#                 st.write(f"**Type:** {inc['properties']['type']}")
#                 st.write(f"**Description:** {inc['properties'].get('description', 'No details')}")
#                 st.write("---")
